		            ...
		            if(result.length > 0) {
					    ...
					    $.each(result, function(i, field){
						    ...
						    newRowTboby += "<tr>";
						      ...
						      newRowTboby += "<td align=\"center\"><button onclick=\"window.opener.kiosWindowLocation ('http://<?php echo $_SERVER['SERVER_NAME']?>/nano/index.php?option=com_rec&act=show&hn=" + field.hn + "&kios_id=" + field.kios_id +"');window.close();\" class=\"btn btn-primary btn-xs\">เลือก</button></td>";
						    newRowTboby += "</tr>";
		...
		<table width="100%" align="center" class="table table-bordred table-striped" id="tb_view_kios" cellspacing="0">
			...
			<?php if ($count_rows > 0):?>
				...
				<tr <?php echo ($i % 2 == 0)?NULL:"class=\"odd\""?> >
			      ...
			      <td align="center"><button onclick="window.opener.kiosWindowLocation ('http://<?php echo $_SERVER['SERVER_NAME']?>/nano/index.php?option=com_rec&act=show&hn=<?php echo "{$row->hn}&kios_id={$row->kios_id}"?>');window.close();" class="btn btn-primary btn-xs">เลือก</button></td>
			    </tr>
			    <?php $i++; endforeach;?>
			<?php endif;?>
		  	</tbody>
