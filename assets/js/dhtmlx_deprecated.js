...
dhtmlXTreeObject.prototype.addPath = function(b, e, c, h) {
	...
	while (a != g) {
		d.push({
			...
			to : (g ? g.id : null),
			style : "border-left:" + (c.width || 1) + "px "
					+ (c.mode || "solid") + " " + (c.color || "red")
					+ "; border-bottom:" + (c.width || 1) + "px "
					+ (c.mode || "solid") + " " + (c.color || "red") + ";"
		});
		g = f;
		f = f.parentObject
...
dhtmlXTreeObject.prototype.activatePaths = function(a) {
	...
	if (a) {
		...
		if (this._idpull[0].childsCount) {
			this._halfHeight = Math
					.floor(this._idpull[0].childNodes[0].span.parentNode.offsetHeight / 2)
		}
	}
	if (!this._halfHeight) {
		this._halfHeight = 9
...
dhtmlXTreeObject.prototype._renderPath = function(g) {
	...
	for (var b = g.path.length - 1; b > 0; b--) {
		...
		h.style.cssText = "position:absolute; z-index:1; width:"
				+ (_isIE ? 10 : 8) + "px; height:" + (j - 9) + "px; left:" + e
				+ "px; top:" + f + "px;" + a.style;
		f += j;
		e += 18;
...
dhtmlXTreeObject.prototype._renderState = function() {
	...
	this
			._getItemByPos(b, this.itemHeight, a, null, false,
...
dhtmlXTreeObject.prototype._renderItemSRND = function(k, j) {
	if (!k.span) {
		...
		for (var c = 1; c < f.length; c++) {
			l = f[c];
			var g = l.nodem ? this.itemHeight
					: (l.offsetHeight || parseInt(l.childNodes[1].firstChild.style.height));
			e -= g;
			if (e < 0) {
				if (e == -1) {
					e++;
				...
				d.style.height = (parseInt(d.style.height) - (g - Math.abs(e) + this.itemHeight))
						+ "px";
				if (Math.abs(e) != g) {
...
dhtmlXTreeObject.prototype._singleTimeSRND = function() {
	...
	this._findNodeByLabel = function(b, f, d) {
		var b = b.replace(new RegExp("^( )+"), "").replace(new RegExp("( )+$"),
				"");
		b = new RegExp(b.replace(/([\*\+\\\[\]\(\)]{1})/gi, "\\$1").replace(
				/ /gi, ".*"), "gi");
		if (!d) {
			d = this._selected[0];
			if (!d) {
	...
	this.deleteChildItems = function(b) {
		if (this.rootId == b) {
			...
			this.allTree
					.insertBefore(this.htmlNode.htmlNode, this.selectionBar);
			if (_isFF) {
...
dhtmlXGridObject.prototype.makePivot = function(j, a) {
	...
	if (a.column_list) {
		...
		for (var g = 0; g < this.hdr.rows[1].cells.length; g++) {
			this._column_list
					.push(this.hdr.rows[1].cells[g][_isIE ? "innerText"
							: "textContent"])
		}
	}
	var h = this;
	...
	var c = this.makePivotSelect([ "Sum", "Min", "Max", "Average", "Count" ],
			-1);
	c.style.width = "70px";
	c.onchange = function() {
		if (this.value != -1) {
...
dhtmlXGridObject.prototype._renderPivot2 = function() {
	...
	for (var c = 0; c < a.length; c++) {
		for (var b = 0; b < e.length; b++) {
			this._pgrid.cells2(b, c + 1)
					.setValue(
							Math
									.round(d(this._pivotS.x, this._pivotS.y,
											this._pivotS.value, a[c], e[b],
											this._pData) * 100) / 100)
		}
	}
};
...
function eXcell_wbut(a) {
	...
	this.edit = function() {
		...
		this.obj.style.height = (this.cell.offsetHeight - (this.grid.multiLine ? 5
				: 4))
				+ "px";
...
function eXcell_liveedit(a) {
	...
	this.onFocus = function() {
		var b = this.grid.callEvent("onEditCell", [ 0,
	...
	this.onBlur = function() {
		var b = this.grid.callEvent("onEditCell", [ 2,
				this.cell.parentNode.idd, this.cell._cellIndex ]);
		this.detach()
	};
